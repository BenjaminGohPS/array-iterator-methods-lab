...
isEveryone19OrOlder = devs.every((dev) => {
  const ageAbove18 = year - dev.year >= 19;
  return (dev.year = ageAbove18);
});
...
travelMethodCounts = travelMethods.reduce((entry, travelMethod) => {
  if (entry[travelMethod]) {
    entry[travelMethod]++;
  } else {
    entry[travelMethod] = 1;
  }
  return entry;
},{});
